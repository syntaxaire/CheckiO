...
class Friends:
    def __init__(self, connections):
        self.friends = {}
        # Friendships are stored in this dictionary with the key being the sorted tuple of the two names,
        # and a value of True. Nothing is set to False, friendships are deleted instead.
        for friendship in connections:
            self.friends[tuple(sorted(friendship))] = True

    def add(self, connection):
        if tuple(sorted(connection)) in self.friends:
            return False
        self.friends[tuple(sorted(connection))] = True
        return True

    def remove(self, connection):
        if tuple(sorted(connection)) not in self.friends:
            return False
        del self.friends[tuple(sorted(connection))]
        return True

    def names(self):
        nameset = set()
        # Add all names from all keys to a set - sets don't add duplicates.
        for friendship in self.friends:
            nameset.update({friendship[0],friendship[1]})
        return nameset


    def connected(self, name):
        friendset = set()
        # Check for the name in each key, then add the other name in the key to our answer
        for friendship in self.friends:
            if friendship[0] == name:
                friendset.add(friendship[1])
            if friendship[1] == name:
                friendset.add(friendship[0])
        return friendset
